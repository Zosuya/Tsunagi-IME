...
///
/// # Shift 系列也要給——但只給「半形回頭路」
///
/// 使用者裁決（2026-09-04）原本是：Shift 系列的符號是明確按出來的，
/// 意圖很清楚（打 `!` 就是要驚嘆號），不需要再選。
///
/// **那個裁決只想到「要不要全形」，漏了反向**（2026-09-08 實測回報）：
/// `Auto` 模式下中文旁邊會自動轉全形，`:` 變成 `：`、`(` 變成 `（`，
/// 而沒有候選就**沒有回頭路**——使用者得刪掉重打。踩到的情境跟 §2.63
/// 修的逗號句號一模一樣：中文句子裡夾程式碼、網址、API 名稱、時間
/// （`參數:值`、`func()`、`真的?`）。
///
/// 所以這五個（`: ? ! ( )`）也給候選，但**只給兩個**：全形（預設，
/// 中文句子裡那是對的）與半形。不像 `[` 那樣列出一整排——那是因為
/// 括號真的有七種寫法要挑，而冒號只有全半形之分。
///
/// # 一鍵兩用不是問題
///
...
pub fn variants(keys: &str, lang: Option<Language>) -> &'static [char] {
    ...
    match c {
        '[' => &['「', '『', '【', '〔', '［', '《', '〈'],
        ']' => &['」', '』', '】', '〕', '］', '》', '〉'],
        // **雙引號也放進單引號的候選**——中文最常用的引號是「」，
        // 但那在 `[` 上；退而求其次的 “” 現在只有按 Shift 打 `"` 才有，
        // 而 `'` 與 `"` 是同一個實體鍵。使用者實測回報「要按 Shift 才
        // 有」，那多一次按鍵的成本比多幾個候選高。
        //
        // 順序是**先單後雙**：這個鍵沒按 Shift，使用者要單引號的機會
        // 大一些；雙引號緊接在後，翻一格就到。
        '\'' => &['‘', '’', '“', '”', '『', '』'],
        // Shift 打出來的雙引號：全形在前，跟其餘 Shift 系列一致
        '"' => &['“', '”', '「', '」', '"'],
        // 中日的第一個不同——逗號是讀點還是逗號，跟 `to_cjk_punct` 一致
        //
        // **半形擺最後**：預設仍是全形（中文句子裡那是對的），但選得到。
        // 中文句子裡夾程式碼、網址、API 名稱時使用者要的就是半形，
        // 而 `/` 與 `-` 本來就把半形放在候選裡——少了這三個是遺漏。
        ',' if ja => &['、', '，', '；', ','],
        ',' => &['，', '、', '；', ','],
        '.' => &['。', '…', '．', '‧', '.'],
        ';' => &['；', '：', ';'],
        ...
        '/' => &['/', '／', '？', '・'],
        '-' => &['-', '－', '—', '～', '‧'],
        // `\` 與 `|` 是同一個實體鍵，跟 `'`／`"` 一樣讓不按 Shift 也拿得到
        '\\' => &['\\', '|', '＼', '｜'],
        '|' => &['|', '｜', '\\', '＼'],
        // ── Shift 系列：全形在前（`Auto` 的預設），半形是回頭路 ──
        ':' => &['：', ':'],
        '?' => &['？', '?'],
        '!' => &['！', '!'],
        '(' => &['（', '('],
        ')' => &['）', ')'],
        // 書名號在 `[`／`]` 的候選裡排第 6，要翻很久。`<`／`>` 這個鍵
        // 中文排版本來就沒有別的用途，直接給它。
        '<' => &['《', '〈', '＜', '<'],
        '>' => &['》', '〉', '＞', '>'],
        // 波浪號：中文表範圍用全形（「三～五天」），這個常用
        '~' => &['～', '〜', '~'],
        // **`@ # $ % & * = + _ { } ^ ` ` `` 刻意不給候選。**
        //
        // 它們的全形版本（＠＃％＆＊）中文排版幾乎沒人用，而這些鍵
        // 正是打程式碼、網址、API 名稱時最常按的——多一頁候選是純粹
        // 的干擾。這跟本函式開頭那條原則一致，也被
        // `compose::tests::標點有候選` 守著（「沒有變體的符號維持
        // 不可選」）。
        //
        // 第一版把它們全加進去，那個測試立刻掛了——**測試擋下的是
        // 對的東西**：`@` 可選之後按方向鍵會多停一格。
        _ => &[],
    }
}
...
mod tests {
    ...
    /// **逗號句號分號都要選得到半形**（使用者實測回報，2026-09-07）。
    ///
    /// 「這個傳給api，這樣知道了嗎」——中文句子裡夾 API 名稱、網址、
    /// 程式碼時使用者要的是半形，但候選裡只有 `，、；` 三個全形，
    /// 選不到就得刪掉重打，正是 `variants` 的文件說要解決的那件事。
    ///
    /// `/` 與 `-` 本來就把半形放在候選第一個，少了這三個是遺漏。
    /// **半形擺最後**：預設維持全形（中文句子裡那是對的）。
    #[test]
    fn 標點要選得到半形() {
        use Language::{Bopomofo, Romaji};
        for (key, lang) in [
            (",", Some(Bopomofo)),
            (",", Some(Romaji)),
            (".", Some(Bopomofo)),
            ...
            let v = variants(key, lang);
            let half = key.chars().next().unwrap();
            assert!(
                v.contains(&half),
                "`{key}`（{lang:?}）的候選要有半形：{v:?}"
            );
            // 預設仍是全形——半形是「選得到」不是「變成預設」
            assert_ne!(v[0], half, "`{key}` 的預設不該是半形");
        }
        // `/` 與 `-` 的半形本來就在第一個，這次沒動到
        assert_eq!(variants("/", Some(Bopomofo))[0], '/');
        assert_eq!(variants("-", Some(Bopomofo))[0], '-');
    }

    /// **Shift 系列的標點也要選得到半形**（使用者實測回報，2026-09-08）。
    ///
    /// `Auto` 模式下中文旁邊自動轉全形（`:` → `：`），而這五個原本
    /// 一個候選都沒有，等於**沒有回頭路**——`參數:值`、`func()`、
    /// `真的?` 這些都得刪掉重打。
    #[test]
    fn shift系列的標點也要選得到半形() {
        use Language::Bopomofo;
        for key in [":", "?", "!", "(", ")"] {
            let v = variants(key, Some(Bopomofo));
            let half = key.chars().next().unwrap();
            assert!(!v.is_empty(), "`{key}` 該有候選，不然半形選不到");
            assert!(v.contains(&half), "`{key}` 的候選要有半形：{v:?}");
            // 預設仍是全形——中文句子裡那才是對的
            assert_ne!(v[0], half, "`{key}` 的預設不該是半形");
            // 每個候選都要能轉回這個鍵，不然是放錯了
            assert_eq!(v.len(), 2, "`{key}` 只需要全形與半形兩個：{v:?}");
        }
    }

    /// **`Auto` 轉出來的全形，一定要在候選裡找得到**。
    ///
    /// 這是前兩個測試背後真正的性質：中文旁邊會被轉成什麼，使用者就
    /// 該選得回去。漏掉任何一個鍵都是 §2.63／§2.68 那種洞。
    #[test]
    fn 自動轉出來的全形都要選得回半形() {
        use Language::Bopomofo;
        for key in [",", ".", ";", ":", "?", "!", "(", ")"] {
            let c = key.chars().next().unwrap();
            let converted = convert(c, Width::Auto, Some(Bopomofo));
            if converted == c {
                continue; // 沒被轉全形的不必給回頭路
            }
            let v = variants(key, Some(Bopomofo));
            assert!(
                v.contains(&c),
                "`{key}` 在中文旁邊變成 `{converted}`，卻選不回半形：{v:?}"
            );
        }
    }
    ...
    /// **同一個實體鍵的兩個符號要互相看得到。**
    ///
    /// 使用者實測回報：`"` 要按 Shift 才打得到。`'`／`"`、`\`／`|`
    /// 都是同一個鍵的上下檔，少按一次 Shift 比多幾個候選划算。
    #[test]
    fn 同鍵的上下檔互相收進候選() {
        for (key, want) in [
            // 沒按 Shift 的 `'` 要看得到雙引號
            ...
            let v = variants(&key.to_string(), None);
            assert!(v.contains(&want), "`{key}` 的候選該有 `{want}`：{v:?}");
        }
    }

    /// **打程式碼常用的符號不給候選**——多一頁候選是干擾。
    ///
    /// 第一版把 `@ # % & *` 全加進去，`compose::tests::標點有候選`
    /// 立刻掛了（「沒有變體的符號維持不可選」）。**測試擋下的是對的
    /// 東西**：可選之後按方向鍵會多停一格。
    #[test]
    fn 程式碼符號不給候選() {
        for c in ['@', '#', '$', '%', '&', '*', '=', '+', '_', '^', '`'] {
            assert!(
                variants(&c.to_string(), None).is_empty(),
                "`{c}` 不該有候選——它是打程式碼用的"
            );
        }
    }
