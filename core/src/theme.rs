...
//! # 顏色為什麼只存 RGB
//!
//! 設定檔寫的是 `"#RRGGBB"`（跟 CSS 一樣，人看得懂）。各平台的原生
//! 顏色型別順序不一（Win32 的 `COLORREF` 是 BGR），**那個轉換是平台的事**
//! ——Windows 的在 `platform/windows/src/theme.rs` 的 `ToColorRef`。
//! 這一層只放「角色叫什麼、值是多少」，不知道誰要拿去畫。

/// 一個顏色，存 RGB。轉成平台原生型別是平台層的事。
...
impl Color {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        crate::render::Rgb::parse_hex(s).map(Self::from)
    }

    /// 轉成 `core` 的平台無關顏色，餵給共用的繪製決策。
    pub const fn to_rgb(self) -> crate::render::Rgb {
        crate::render::Rgb::new(self.r, self.g, self.b)
    }
}

impl From<crate::render::Rgb> for Color {
    fn from(c: crate::render::Rgb) -> Self {
...
pub struct Metrics {
    /// 反白條的樣式（實心／高光帶／只有高光）。
    pub highlight_style: crate::config::HighlightStyle,
...
/// 靠註解提醒「記得對齊」，那種對齊遲早失守（見 `crate::render`）。
pub use crate::render::fixed;
...
pub struct Theme {
    ...
    pub background: crate::config::Background,
}
...
mod tests {
    ...
    #[test]
    fn 縮放百分之百時等於基準值() {
        let t = Theme::from_config(&crate::config::Config::default());
