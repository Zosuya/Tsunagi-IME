...
/// 底部提示列的字級，**依候選字的字級換算**。
///
/// 提示列是輔助資訊（`↑↑↓↓ ⚙ 開啟設定`、`日文已停用`），比候選字**小一號**
/// ——但只小一號。原本兩個平台各寫死 `4/5`（12pt → 9pt），實際看下來太小，
/// 尤其在 macOS 上：同一個 pt 值在那邊的視覺尺寸本來就比 Windows 小。
///
/// 下限 9pt：再小就不是「輔助」而是「看不見」。
///
/// **放在這裡是因為它是繪製的決策**，不是某個平台的實作細節——兩個平台
/// 各寫一份 magic number 的話，改了一邊就默默分岔。
pub fn hint_font_pt(font_size_pt: i32) -> i32 {
    (font_size_pt * 11 / 12).max(9)
}
