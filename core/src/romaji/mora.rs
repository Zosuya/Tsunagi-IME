...
fn check_body(c: &[char], i: usize) -> Validity {
    ...
    //
    // **例外是 `ny` 開頭的拗音**（にゃ／にゅ／にょ）。這裡不能無條件
    // 判非法——`nya` 的第二字元是 `y` 不是母音，會在輪到下面的拗音
    // 規則之前就被殺掉，整個 n 行拗音因此打不出來（§2.76）。
    // 撥音那一族不受影響：`nn` 早在 check() 就回傳了，走不到這裡。
    if first == 'n' && i == 1 && x != 'y' {
        // `n` + 非母音非 y → 撥音，但後面不能再有東西
        // （`nn` 已在 check() 處理）
        return Validity::Invalid;
    }
...
mod tests {
    ...
    fn 拗音() {
        for k in [
            ...
            // n 行：§2.76 的缺陷就活在這個盲區裡——原本整行沒被測到
            "nya", "nyu", "nyo", "mya", "hya", "rya",
        ] {
            assert!(v(k), "{k:?}");
        }
    }
    ...
    /// §2.76：`ny` 是拗音的例外，但**只有 `ny`＋母音**那一種。
    ///
    /// 放行的是「落到拗音規則去判斷」，不是「n 開頭都合法」——所以
    /// `ny` 沒接母音、或 `n` 接別的子音，仍然非法。
    #[test]
    fn n_開頭只有拗音是例外() {
        for k in ["nya", "nyu", "nyo"] {
            assert!(v(k), "{k:?} 是拗音");
        }
        // `ny` 打到一半、或接了非母音 → 還是非法
        assert!(!v("ny"), "ny 還沒打完");
        assert!(!v("nyk"), "ny 後面不是母音");
        // `n` 接別的子音仍是撥音那條路（非法，撥音要打 nn）
        assert!(!v("nk"), "nk");
        assert!(!v("nt"), "nt");
        // 4 字元不是一個 mora（175 條裡沒有 4 字母的，見檔頭）
        assert!(!v("nnya"), "nnya 要切成 nn+ya");
    }
