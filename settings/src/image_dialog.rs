...
//! # macOS
//!
//! **還沒實作**——`NSOpenPanel` 或 `rfd`，等 §2.52.8 待決 2 的後半拍板。
//! 現在回 `None`（等同使用者按取消），設定頁的其他部分照常運作。
...
#[cfg(target_os = "macos")]
mod imp {
    /// macOS：系統的「選檔案」面板，只讓選圖片。
    ///
    /// **不給起始目錄**——Windows 版也沒給，系統會記得上次去過哪裡，
    /// 那比每次都跳回同一個地方好用。
    pub fn choose() -> Option<String> {
        crate::mac_panel::pick(crate::mac_panel::Want::Image, None)
    }
}
