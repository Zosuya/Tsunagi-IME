...
//! # macOS
//!
//! **還沒實作**。`rfd` 沒有字型選擇器，所以這一支不管怎樣都要手刻
//! `NSFontPanel`——它是 modal-less 的面板，跟這裡「叫出來、等使用者選完、
//! 拿回結果」的同步介面不合，要改成回呼或自己跑 modal session。
//! 現在回 `None`（等同取消），字型維持原設定。
