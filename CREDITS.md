...
台語包**不隨安裝程式散布**（2026-09-11 決定，見開發筆記），放在獨立的
[Tsunagi-packs](https://github.com/Zosuya/Tsunagi-packs)，使用者自己下載
放進擴充包資料夾。它跟上面的詞庫不同——**原始資料的內容直接在裡面**。

發佈兩種格式，**內容相同**：`台語.txt`（純文字，看得到也改得動）與
`台語.bin`（二進位，載入快、跨行程共用同一份記憶體）。授權義務對兩者
一視同仁——`.bin` 的檔頭一樣存著姓名標示那幾行。

| 資料 | 來源 | 授權 |
|---|---|---|
| 華語↔台語對照<br>`台語.txt`／`台語.bin` | [ChhoeTaigi 找台語](https://github.com/ChhoeTaigi/ChhoeTaigiDatabase)｜2002+ 台文華文線頂辭典<br>基礎資料：鄭良偉 教授；增補校訂：楊允言 教授與眾義工 | CC BY-SA 4.0 |
