...
那是一個優閒的下午，我用著電腦打著日文的字幕翻譯「怎麼有人名又有專有名詞阿，煩死了。」「乾rup wu0 wu0 fu4cp3cl3xk7這是甚麼外星文?」，在越打越牙的情況下，決定來製作這款輸入法。
這是一款能夠自動辨識輸入語言（中文注音／英文／日文羅馬字）的輸入法，讓你不用再打出台灣特有加密文字的時候一直⭠Backspace，或是在打辛苦打一串英文之後才發現怎麼都是ㄍㄩㄛˊ
...
# 功能
除了混合語言輸入以外，還有一些特別的設計來解決目前輸入法會遇到的一些問題與使用體驗。
...
在通譯輸入法中，他會學習你更正過的選字並記錄下來，你用的越多次，他會記得越深。不會是一次就學會，避免你選了一個生僻字之後就再也打不出一般的字了。
...
支援自訂主題，你可以調整預選框的顏色，樣式等等，甚至還可以使用圖片。所有的設定也都是純文字檔，你可以把你製作的夢幻芭比粉主題分享給你的好兄弟用。
...
可以利用想輸入的類型直接在輸入框樹入並搭配快速設定組合鍵來改變設定。例如你可以用任何語言輸入「設定」之後 再使用↑ ↑ ↓ ↓來快速進入輸入法的設定頁面，或是輸入語言來快速啟用和停用。
...
你可以利用上面的快速設定或是設定頁面中，關閉不需要的語言，這樣在混和輸入中被關閉的語言就不會餐與辨識，可以讓剩下的語言辨識準確度大幅提升。
# 隱私權聲明

- **密碼欄位不介入**-本輸入法不介入任何密碼欄位，也不學習，一切輸入接直接送進文字欄中。
- **所有資料接存於本機**-你所透過本輸入法所輸入的任何文字皆存於本地電腦中，不會推送到任何網路伺服器中。
# 現況
