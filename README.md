...
**用注音打華語詞，選出台語漢字。**打「ㄕㄚㄈㄚ」得到「沙發」，開段選單就能換成「膨椅」。

台語擴充包收了約 5 萬條華語↔台語的對照（資料來自[台文華文線頂辭典](https://github.com/ChhoeTaigi/ChhoeTaigiDatabase)），走的是**段選單**——不佔平常打字的候選位置，只在你主動打開時才出現，也不必切換模式。反白的單位是「詞」，`←→` 跳詞、`Shift+←→` 調整範圍。
